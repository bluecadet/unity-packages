...
//! scalar accessors -- the convention this codebase uses for every
//! non-trivial C API.
...
/// encapsulating the bounds guard and header-advance logic every child
/// lookup shares.
...
/// Find the `n`th direct child in `[start, end)` whose type matches
/// `fourcc`, counting from zero; null if there are fewer than `n + 1` of
/// them. Boxes of every other type are skipped, so `n` indexes the matches,
/// not the children.
fn findChildAt(file_data: []const u8, start: u64, end: u64, fourcc: u32, n: u32) ?BoxHeader {
...
fn findChild(file_data: []const u8, start: u64, end: u64, fourcc: u32) ?BoxHeader {
    return findChildAt(file_data, start, end, fourcc, 0);
}
...
/// the display edge (the C ABI maps these to HapError codes, which OpenResult
/// on the C# side turns into text), which lets a caller special-case
/// UnsupportedHapVariant against the offending fourcc parked on
...
/// What a single `stsd` box turned out to hold, as returned by `parseStsd`.
/// Null (no `StsdMatch` at all) means the box carries no Hap sample entry;
/// the two tags carry exactly the data that outcome produces, so nothing
/// reads a field that outcome didn't fill in.
pub const StsdMatch = union(enum) {
    /// A supported Hap variant, with its fourcc and dimensions.
    found: VideoFormat,
    /// A Hap codec this extension recognizes but cannot decode (HapA, Hap
    /// HDR). Dimensions are not read; only the fourcc, for the error message.
    unsupported: FourCC,
};
...
pub const Demuxer = struct {
    ...
    /// Releases the cached sample table. Safe to call on a zero-value
    /// Demuxer.
    ...
    pub fn parseStsd(data: []const u8) ?StsdMatch {
        ...
        while (i < entry_count) : (i += 1) {
            ...
            if (entry_size < visual_sample_entry_min_size) return null;

            const entry_end = std.math.add(usize, offset, entry_size) catch return null;
            ...
            if (hap_frame.classify(fourcc) != null) {
                ...
                return .{ .found = .{
                    .fourcc = fourcc,
                    .width = std.mem.readInt(u16, data[dim_offset..][0..2], .big),
                    .height = std.mem.readInt(u16, data[dim_offset + 2 ..][0..2], .big),
                } };
            }
    ...
    pub fn open(self: *Demuxer, allocator: std.mem.Allocator, reader: *const MmapReader) OpenError!void {
        ...
        // Everything minimp4 knows is copied into `self.samples`/`self.track`
        // before this returns, so its context lives no longer than the call.
        const ctx = hap_mp4d_open(file_data.ptr, @intCast(file_size)) orelse return error.MalformedMp4;
        defer hap_mp4d_close(ctx);
        ...
        while (t < track_count) : (t += 1) {
            const trak = findChildAt(file_data, moov.data_pos, moov.offset + moov.size, fourcc_trak, t) orelse continue;
            const stsd = findStsdInTrak(file_data, trak) orelse continue;

            const stsd_data = file_data[stsd.data_pos..][0..stsd.data_size];

            switch (parseStsd(stsd_data) orelse continue) {
                .found => |format| {
                    hap_format = format;
                    hap_track_index = t;
                    break;
                },
                .unsupported => |fourcc| unsupported_hap_fourcc = fourcc,
            }
        }
