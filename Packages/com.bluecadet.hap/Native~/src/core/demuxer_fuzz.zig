...
//! production use), but its `data` field is public, so this harness builds
//! an in-memory `MmapReader{ .data = ... }` directly -- functionally
//! identical to a real mmap from Demuxer.open()'s point of view (it only
//! ever reads through `reader.data`) and avoids per-iteration file I/O.
//!
...
fn fuzzOpen(context: void, smith: *testing.Smith) !void {
    ...
    var reader: MmapReader = .{ .data = buf[0..len] };
    var dem: Demuxer = .{};
...
test "fuzz Demuxer.open on arbitrary bytes" {
    var readers: [fixture_paths.len]MmapReader = undefined;
    var opened: usize = 0;
    ...
    // Plus the fuzz-found regression corpus, enumerated once in
    // test_support so this harness, hap_decode_fuzz.zig and
    // fuzz_regressions_test.zig can't drift apart on which files it holds.
    ...
    try testing.fuzz({}, fuzzOpen, .{ .corpus = corpus.items });
}
...
test "bounded randomized fuzz (opt-in via HAP_FUZZ_SECONDS)" {
    ...
    // stderr (std.debug.print), never stdout: stdout is the test runner's
    // `--listen=-` IPC channel with `zig build`. Note that any stderr output
    // at all makes the build runner echo the step with a "failed command:"
    // context line even though the step succeeded -- cosmetic, the build
    // still exits 0 (checked on zig 0.16.0).
    std.debug.print(
