...
{
    /// <summary>
    /// What the background open thread produces: either a live native handle plus the track's
    /// metadata, or the native reason the file could not be played.
    /// </summary>
    internal sealed class NativeOpenResult
    {
        ...
        public NativeOpenResult(IntPtr handle, int frameCount, float frameRate,
        ...
        NativeOpenResult(HapError error)
        {
            Handle   = IntPtr.Zero;
            Error    = error;
        ...
        public static NativeOpenResult Failed(HapError error) => new(error);
    }
}
